...
18. **About page reworked to be personality-first (2026-09-06).** User's brief: lead with who they are, not a stacks-and-skills resume — movies, video games, cooking, drumming — with the factual blocks kept lower on the same page. Of three concepts offered (Four Obsessions poster cards / The Personal Stack / Now Playing board), the user chose the **Now Playing board**.
    ...
    - **First execution was rejected** — it was an ink `#0A0A0A` board panel holding four identical white cards that differed only in their text. User feedback: the black background was wrong, and four identical rectangles are not differentiation. Quote: *"a user won't understand I'm playing a video game at a glance"* and *"the cards all need to be slightly unique in their own way as they are in the end different hobbies — that differentiation is also what should be making them a recognisable element."* The dark panel was deleted and the whole thing rebuilt.
    - **Current build: four cards, each a different physical object from its own world**, sitting directly on the paper ground in a collage (no panel). Section is eyebrow `OFF THE CLOCK` + Archivo Black 44 `WHAT I'M INTO RIGHT NOW` with an `UPDATED SEP 2026` stamp right-aligned on the title row, then a 1288x560 `Collage` frame (node `2213:9`), then a hairline rule and the through-line. All cards are white, radius 0, 2.5px ink border, 8px hard ink offset shadow.
      ...
      - **Cooking — recipe index card** (`2217:2`, 348x336). Cream header strip with a **steaming-pot** glyph and a red rule under it; a red vertical **margin line** down the left like real index card stock; a checklist of steps with square checkboxes (2 filled red, 1 empty) separated by ruled hairlines.
      - **Drumming — notation strip** (`2218:2`, 940x192, wide, sits under Games+Cooking). Red accent spine down the left, a **crossed drumsticks** glyph, and on the right an actual **4-bar drum staff** — three staff lines, barlines, 16 hi-hat × marks, snare and kick note heads — next to a `96 BPM` readout with a red pulse dot.
    - Sprocket holes and the tear line are single `LINE` nodes with `dashPattern` rather than dozens of rects — cheap to build and to edit. Rotated elements are positioned by comparing `absoluteBoundingBox` before/after so the rotation pivot never matters.
    - **The argument the section makes:** all four hobbies are timing-and-composition disciplines (editing, feedback loops, seasoning, tempo), so it reads as a claim about how the user designs rather than a hobby list. Closing line, with `TIMING.` in red: `ALL FOUR ARE THE SAME SKILL — TIMING. I DESIGN THE WAY I PLAY.`
    - **Content is dummy and must be swapped** — Whiplash / Disco Elysium / Butter Chicken / Rosanna Shuffle, plus the star rating, 62% completion, checklist steps and BPM. Waiting on the user's real specifics.
    - Three **image slots** to fill manually (plugin API blocks `createImageAsync`): film poster, game screenshot. Cooking and drumming cards are drawn entirely in vector and need no image.
    ...
    - Not built: hover states for these cards, and no entry added to the `Cursor & Hover States` frame for them.
